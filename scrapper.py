...
# pythons library for getting individual items from website
...
def checking_price():
    ...
    # finds the title tag in html
    title = soup.find(id="productTitle").get_text()

    # make sure its getting the right information, this should be the product title
    print(title.strip())

    # this is a string and needs to be converted to a number
    for prices in soup.find('span', {'class': 'a-offscreen'}):
        price = str(prices.text)
        print(price)

    # converts the price to a number and takes away the $ and decimals
    converted_price = int(price[1:-3])

    print(converted_price)  # make sure everything is looking good

    if(converted_price < 200):  # if its below the price we want, calls send_email function
        send_email_google()
...
def send_email_google():
    ...
    try:
        ...
        # IMPORTANT NOTE!!
        #
        # Google has security measures in place to not let certain apps have access to send emails
        # Make sure to go to your account and allow access for less secure apps and make sure its turned on.
